...
export class PublicAction {
    ...
    public async loginAction(
        ...
        const user = await cds.ql.SELECT.from(vacation.Entity.Users).where({
            username: username,
        });
        ...
        const refreshToken = await generateRefreshToken(user[0]);

        

        const updatedUser = await cds.ql.UPDATE("Users").where({ ID: user[0].ID }).set({ refreshToken: refreshToken });
        if (!updatedUser) {
